...
export function ActionModalView({
  ...
  const { maxUserBorrow, isLoadingMaxUserBorrow, refetchMaxUserBorrow } =
    useReadMaxUserBorrow(market.id as `0x${string}`, tokenDecimalsForHooks);
    ...
    if (type === "supply_collateral" || type === "supply_liquidity") {
      ...
      if (
        ...
        const maxBorrowAmount =
          Number(maxUserBorrow) / Math.pow(10, tokenDecimalsForHooks);

        if (borrowAmount > maxBorrowAmount) {
          toast.error("Borrow amount exceeds your maximum borrow limit", {
            description: `Maximum borrow amount: ${formatMaxAmount(
              maxBorrowAmount
            )}`,
            duration: 5000,
            style: {
