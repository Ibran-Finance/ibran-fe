                  ...
                  {lpAddress.find(lp => lp.id === lpAddressSelected) && (
                    <>
                      <div className="flex items-center gap-1 md:gap-2 flex-1">
                        <Image
                          src={tokenLogo(lpAddress.find(lp => lp.id === lpAddressSelected)?.collateralToken) ?? ""}
                          alt={tokenName(lpAddress.find(lp => lp.id === lpAddressSelected)?.collateralToken) ?? ""}
                          className="size-4 md:size-6 rounded-full flex-shrink-0"
                          width={16}
                          height={16}
                        />
                        <span className="text-xs md:text-sm truncate">
                          {tokenName(lpAddress.find(lp => lp.id === lpAddressSelected)?.collateralToken)}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-1 md:gap-2 flex-1 justify-end">
                        <Image
                          src={tokenLogo(lpAddress.find(lp => lp.id === lpAddressSelected)?.borrowToken) ?? ""}
                          alt={tokenName(lpAddress.find(lp => lp.id === lpAddressSelected)?.borrowToken) ?? ""}
                          className="size-4 md:size-6 rounded-full flex-shrink-0"
                          width={16}
                          height={16}
                        />
                        <span className="text-xs md:text-sm truncate">
                          {tokenName(lpAddress.find(lp => lp.id === lpAddressSelected)?.borrowToken)}
                        </span>
                      </div>
                    </>
                  )}
