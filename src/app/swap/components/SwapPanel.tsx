                    ...
                    {lpAddress.find(lp => lp.id === lpAddressSelected) && (
                      <>
                        <div className="flex items-center gap-1 md:gap-2 flex-1">
                          <Image
                            src={lpAddress.find(lp => lp.id === lpAddressSelected)?.collateralTokenInfo?.logo ?? ""}
                            alt={lpAddress.find(lp => lp.id === lpAddressSelected)?.collateralTokenInfo?.name ?? ""}
                            className="size-4 md:size-5 rounded-full flex-shrink-0"
                            width={16}
                            height={16}
                          />
                          <span className="text-xs md:text-sm truncate">
                            {lpAddress.find(lp => lp.id === lpAddressSelected)?.collateralTokenInfo?.name ?? "Unknown"}
                          </span>
                        </div>
                        ...
                        <div className="flex items-center gap-1 md:gap-2 flex-1 justify-end">
                          <Image
                            src={lpAddress.find(lp => lp.id === lpAddressSelected)?.borrowTokenInfo?.logo ?? ""}
                            alt={lpAddress.find(lp => lp.id === lpAddressSelected)?.borrowTokenInfo?.name ?? ""}
                            className="size-4 md:size-5 rounded-full flex-shrink-0"
                            width={16}
                            height={16}
                          />
                          <span className="text-xs md:text-sm truncate">
                            {lpAddress.find(lp => lp.id === lpAddressSelected)?.borrowTokenInfo?.name ?? "Unknown"}
                          </span>
                        </div>
                      </>
                    )}
          ...
          lpAddressSelected && (
            <div className="bg-yellow-900/20 border border-yellow-500/30 rounded-xl p-4 text-yellow-400">
              ...
              <p className="text-sm text-yellow-300">
                You need to create a position first by supplying collateral and
                borrowing assets. Visit the{" "}
                <span className="font-medium">Borrow</span> page to get started.
              </p>
            </div>
          )}
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            <Select
              ...
              onValueChange={(value) =>
                setFromToken(
                  tokens.find((t) => t.addresses[defaultChain] === value) ||
                    tokens[0]
                )
              }
            >
