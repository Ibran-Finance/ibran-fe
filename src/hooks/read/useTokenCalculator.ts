...
export const useTokenCalculator = (
  ...
  const normalizedAmount = Number.isFinite(amountIn) && amountIn > 0 ? amountIn : 0;
  const roundedAmount = Math.round(normalizedAmount * 10 ** precision) / 10 ** precision;
  const amountInBigInt = BigInt(Math.round(roundedAmount * 10 ** (decimalsIn ?? 0)));
